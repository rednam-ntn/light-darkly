      ...
      <div className="divide-y divide-gray-50">
        {variations.map((v, i) => (
          <div key={v._id} className="flex items-start gap-3 px-5 py-3">
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded bg-gray-100 text-xs font-medium text-gray-500">
              {i}
            </span>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <code className="rounded bg-gray-50 px-2 py-0.5 font-mono text-sm text-gray-800">
                  {formatValue(v.value)}
                </code>
                {v.name && (
